...
功能說明：
1. 使用預訓練的XGBoost模型預測低波動蔬菜未來7天價格
2. 從資料庫自動獲取歷史資料
3. 支援批量日期範圍預測
4. 支援多線程並行處理提高效率
5. 自動更新實際價格和MAPE計算
...
注意：此程式專門處理低波動蔬菜，使用low_volatility_merged表作為資料來源
"""
...
# 蔬菜設定
VEGE_IDS = None                    # 指定蔬菜ID列表，例: [1, 2, 3] 或 None (表示所有可用蔬菜)

# 歷史資料設定
HISTORICAL_DAYS = 180              # 使用多少天的歷史資料進行預測

# 資料庫連接設定
DB_CONNECTION = 'postgresql+psycopg2://lorraine:0000@111.184.52.4:9527/postgres'

# 模型路徑 (None表示使用預設路徑)
MODEL_PATH = None                  # 自訂模型路徑，或 None 使用預設
# =======================================
...
class DatabaseVegetablePricePredictor:
    ...
    def get_available_vegetables(self) -> List[int]:
        ...
        查詢low_volatility_merged表中有歷史資料的蔬菜，
        並與模型中已訓練的蔬菜進行交集

        返回:
            List[int]: 可進行預測的低波動蔬菜ID列表
        """
        try:
            # 查詢low_volatility_merged表中有歷史價格資料的蔬菜
            query = """
            SELECT DISTINCT vege_id
            FROM public.low_volatility_merged
            ...
            for i in range(1, 8):
                ...
                for feature in feature_names:
                    if feature in feat_df.columns:
                        X_dict[feature] = feat_df.iloc[-1][feature]
                    else:
                        X_dict[feature] = 0.0  # 缺失特徵用0填充
                
                # 按照模型期望的特徵順序建立矩陣
                X = pd.DataFrame([X_dict], columns=feature_names).fillna(0.0)
...
def main():
    ...
    初始化低波動蔬菜價格預測器並執行預測任務
    使用models_low_next7.pkl模型檔案
    """
    # 獲取當前腳本所在目錄的絕對路徑
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # 設定低波動蔬菜預測模型的預設路徑
    default_model_path = os.path.join(script_dir, 'models_low_next7.pkl')
